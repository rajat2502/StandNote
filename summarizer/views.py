...
class SummariserCosine:
    ...
    def sentence_similarity(self, first_sentence, second_sentence, stop_words=None):
        ...
        first_vector = self.extract_vector(
            first_sentence, all_words, stop_words)
        second_vector = self.extract_vector(
            second_sentence, all_words, stop_words)
            ...
            for another_sentence_index, another_sentence in enumerate(sentences):
                ...
                similarity_matrix[this_sentence_index][another_sentence_index] = \
                    self.sentence_similarity(
                        this_sentence, another_sentence, stop_words)
    ...
    def generate_summary(self, text, top_n_sentences):
        ...
        sentence_similarity_graph, scores = self.rank_sentences(
            sentence_similarity_martix)

        # Step 4 - Sort the rank and pick top sentences
        ranked_sentences = self.sort_sentences_to_surface_top_ranked_sentences(
            scores, sentences)

        # Step 5 - Construct the summarised text
        summarised_text = self.summarise_text(
            ranked_sentences, top_n_sentences)
